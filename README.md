...
Отчет создается захватывая дни с даты полсденего отчета по сегодняшний. 
...
Папка `build/` содержит готовые к использованию файлы плагина (`main.js`, `manifest.json`, `versions.json`). Просто скопируйте её в vault.
...
Ваши ежедневные заметки могут быть в любом формате, но есть несколько правил
...
- **Included Sections**: Список секций (заголовков уровня 2), которые нужно включать в отчёт, разделённые запятыми.
  - Пример: `План, Лог, Блокеры / вопросы, Итого`
